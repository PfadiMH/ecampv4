...
/**
 * Authenticates users coming back from a generic OpenID Connect provider.
 *
 * Users are matched by their stable OIDC subject identifier ("sub" claim), falling back
 * to their email address. Unknown users are provisioned on the fly.
 */
class OidcAuthenticator extends OAuth2Authenticator {
    ...
    public function supports(Request $request): ?bool {
        ...
        return 'connect_oidc_check' === $request->attributes->get('_route');
    }
    ...
    public function authenticate(Request $request): Passport {
        ...
        return new SelfValidatingPassport(
            new UserBadge($accessToken->getToken(), function () use ($accessToken, $client) {
                ...
                $oidcId = (string) $resourceOwner->getId();
                $email = $claims['email'] ?? null;

                $profileRepository = $this->entityManager->getRepository(Profile::class);

                // has the user logged in via OIDC before?
                $existingProfile = $profileRepository->findOneBy(['oidcId' => $oidcId]);
                ...
                if (empty($email)) {
                    throw new CustomUserMessageAuthenticationException('The OIDC provider did not return an email address.');
                }
                ...
                if (is_null($profile)) {
                    $profile = new Profile();
                    $profile->email = $email;
                    $profile->firstname = $claims['given_name'] ?? null;
                    $profile->surname = $claims['family_name'] ?? null;
                    $user = new User();
